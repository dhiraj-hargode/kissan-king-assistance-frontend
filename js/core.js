...
function blankDB(){return {customers:[],loans:[],schedules:[],payments:[],blacklist:[],notifications:[],deletedRecords:[],expiredCustomers:[],pendingQueue:[],settings:{appName:"Kissan-King Assistance",currency:"INR",defaultInterest:2,defaultPenalty:0,reminderDays:[7,3,1,0],logoData:"",logoEnabled:true}}}
...
async function loadPublicBranding(){try{const x=await apiJSON('/api/public/branding');const g=x.branding||{};db.settings={...(db.settings||blankDB().settings),appName:g.appName||db.settings?.appName||'Kissan-King Assistance',logoData:g.logoData||db.settings?.logoData||'',logoEnabled:g.logoEnabled!==false};applyAppBranding();}catch(e){applyAppBranding();}}
