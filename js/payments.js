...
async function showSchedule(loanId){
  try{
    selectedLoanId=String(loanId||"");
    await ensureServerDataLoaded();
    if(!db.loans.some(x=>String(x.id)===String(selectedLoanId))){
      toast("Loan not found.","err");
      return;
    }
    openPage("schedule");
  }catch(e){
    console.error(e);
    toast(e.message||"Could not open repayment schedule","err");
  }
}
