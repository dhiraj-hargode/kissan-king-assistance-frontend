...
async function deletePaymentById(id){
  ...
  if(!confirm('Delete this payment? The record will be moved to Deleted Records for audit.')) return;
  try{
    await apiJSON(`/api/payments/${encodeURIComponent(id)}`,{method:'DELETE'});
    if(typeof paymentHistoryCache!=='undefined') paymentHistoryCache.clear();
    toast('Payment deleted');
    closeModal();
    openPage('history');
  }catch(e){
    toast(e.message||'Could not delete payment','err');
  }
}
